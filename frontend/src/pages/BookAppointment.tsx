    ...
    const fetchInitialData = async () => {
      try {
        ...
        const [clinicRes, petsRes] = await Promise.all([
          fetch(`http://localhost:8000/vets/${clinicId}`, { headers: { Authorization: `Bearer ${token}` } }),
  ...
  useEffect(() => {
    if (bookingData.date) {
      const fetchSlots = async () => {
        ...
        try {
          const res = await fetch(`http://localhost:8000/appointments/slots?clinic_id=${clinicId}&date=${bookingData.date}`);
          if (res.ok) {
